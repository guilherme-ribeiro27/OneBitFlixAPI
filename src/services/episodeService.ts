...
export const episodeService = {
    streamEpisodeToResponse : (res : Response, videoUrl : string, range : string | undefined)=>{
        ...
        const fileStat = fs.statSync(filePath); // dados do arquivo
    ...
    setWatchTime: async({userId, episodeId, seconds}: WatchTimeAttributes)=>{
        const watchTimeAlereadyExists = await WatchTime.findOne({
            attributes:['seconds'],
            where: {
                userId,
                episodeId
        ...
        if(watchTimeAlereadyExists){
            watchTimeAlereadyExists.seconds = seconds;
            await watchTimeAlereadyExists.save();
            return watchTimeAlereadyExists
        }else{
